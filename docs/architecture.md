...
Export currently preserves source topology and rejects degenerate output instead of silently repairing it. Subdivision, adaptive remeshing, broader manifold diagnostics, repair proposals, and OBJ export remain future slices. The next slice should add a preflight diagnostic report and an explicit subdivision/detail strategy before expanding export formats.
