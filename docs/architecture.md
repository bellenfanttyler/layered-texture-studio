...
The eighth slice adds export diagnostics. Import derives angle-weighted normals shared by coincident STL vertices, so duplicated triangle records use one stable displacement direction and remain aligned. The export worker counts boundary and non-manifold edges with scale-relative coordinate keys, measures displaced vertices and maximum displacement, and returns warnings with the validated STL buffer. The current shell exposes one download action; after the browser download begins, advisory findings appear only when present in a dismissible warning dialog.
